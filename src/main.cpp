...
void testaExclusoes(AVL *lstBEAMS[QTD_BEAMS], No_AVL *listaPessoas[QTD_PESSOAS])
{
        ...
        if (lstBEAMS[idBEAM]->T != NULL)
        {
            ...
            if (lstBEAMS[idBEAM]->chaveEncontrada != NULL)
            {
                if (idBEAM >= 0 && idBEAM < QTD_BEAMS)
                {
                    ...
                    //lstBEAMS[idBEAM]->exclui(&lstBEAMS[idBEAM]->chaveEncontrada, telefone);
                    ...
                    lstBEAMS[idBEAM]->imprimeRepresentacaoGraficaArvore((i + 1) * 2);
                }
            }
            else
            {
...
int main()
{
    ...
    testaExclusoes(lstBEAMS, listaPessoas);
    /*
    /* inicializa a semente (seed): 
    srand (time(NULL));
